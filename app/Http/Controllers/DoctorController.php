...
class DoctorController extends Controller
{
    public function dashboard()
    {
        ...
        if ($user->role === 'pedia_doctor') {
            $appointments = $waiting->filter(function($c) { return str_starts_with($c->queue_number, 'APED-'); })->values();
            $walkins = $waiting->filter(function($c) { return str_starts_with($c->queue_number, 'PED-'); })->values();
            $emergencies = $waiting->filter(function($c) { return str_starts_with($c->queue_number, 'PED-E-'); })->values();

            // Push emergencies first
            foreach($emergencies as $e) $queue->push($e);

            $aptIdx = 0;
            $walkIdx = 0;
            while ($aptIdx < $appointments->count() || $walkIdx < $walkins->count()) {
                if ($aptIdx < $appointments->count()) $queue->push($appointments[$aptIdx++]);
                if ($aptIdx < $appointments->count()) $queue->push($appointments[$aptIdx++]);
                if ($walkIdx < $walkins->count()) $queue->push($walkins[$walkIdx++]);
            }
        } else {
            $regIdx = 0;
            $prioIdx = 0;
    ...
    public function startConsultation(Consultation $consultation)
    {
        ...
        $consultation->load(['preTriage', 'ancillaryRequests']);
        ...
        return view('doctor.consultation', compact('consultation', 'patient', 'pastConsultations'));
    }
    ...
    public function completeConsultation(Request $request, Consultation $consultation)
    {
        ...
        \App\Models\MedicalCase::create([
            ...
            'vitals_snapshot' => [
                ...
                'ox'   => $consultation->preTriage->spo2 ?? ($consultation->preTriage->oxygen_saturation ?? null),
            ],
            'closed_at'       => now(),
        ]);
    ...
    public function showPatient(\App\Models\Patient $patient)
    {
        ...
        $hasActiveConsultation = Consultation::where('patient_id', $patient->patient_id)
            ->where('doctor_id', $user->id)
            ->whereIn('status', ['in_consultation', 'queued', 'in_triage'])
    ...
    public function storeAncillaryRequest(Request $request, Consultation $consultation)
    {
        ...
        $consultation->update(['status' => 'active']); // Keep it active for simulation

        return back()->with('success', $validated['type'] . ' request created (Simulation Flow). You can continue the consultation.');
    }
}
