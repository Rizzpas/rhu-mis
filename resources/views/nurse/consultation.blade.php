...
<div class="max-w-7xl mx-auto space-y-6" x-data="{ 
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
            <a href="{{ route('nurse.dashboard') }}" class="p-2 bg-white dark:bg-gray-800 rounded-full text-slate-500 hover:text-teal-600 hover:bg-teal-50 shadow-sm border border-slate-200 transition">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-b-xl rounded-tr-xl shadow-lg border-2 border-teal-600 grow flex flex-col z-0 relative">
                ...
                <div x-show="activeTab === 'current'" 
                    ...
                    <div class="p-5 border-b border-slate-100 bg-white dark:bg-gray-800 rounded-tr-lg">
                        <h3 class="text-lg font-extrabold text-slate-800">Nurse's Assessment</h3>
                    ...
                    <div x-data="{ showAncillary: false }" class="p-6 pb-0">
                        ...
                        <div x-show="showAncillary" x-collapse class="mt-3">
                            <div class="bg-indigo-50 border border-indigo-200 p-4 rounded-lg shadow-inner">
                                <p class="text-xs text-indigo-700 mb-3 font-medium">Instantly forward this patient to the Laboratory or Radiology queue. They will appear on the staff dashboard.</p>
                                <form action="{{ route('nurse.ancillary.store', $consultation->id) }}" method="POST" class="flex flex-col md:flex-row gap-3" x-data="{ requestType: 'Laboratory' }">
                                    @csrf
                                    ...
                                    <select name="test_name" x-show="requestType === 'Radiology'" class="flex-1 text-sm rounded-md border-indigo-300 focus:border-indigo-500 focus:ring-indigo-500 bg-white" x-cloak>
                                        ...
                                        <option value="Chest X-Ray AP View">Chest X-Ray AP View</option>
                    ...
                    <form action="{{ route('nurse.consultation.complete', $consultation->id) }}" method="POST" class="grow flex flex-col p-6 space-y-6" @submit.prevent="showConfirm(
                        'Complete Consultation?', 
                        'Are you sure you want to finalize this medical record? Please ensure all diagnoses and prescriptions are correct.', 
                        () => $el.submit()
                    )">
                        @csrf
                        ...
                        <div x-data="prescriptionBuilder('teal')" class="mb-4 relative">
                            ...
                            @if(!($isPharmacyOnline ?? true))
                            <div class="mb-3 bg-amber-50 border-l-4 border-amber-500 p-3 rounded-md">
                                <div class="flex items-start">
                                    <svg class="h-5 w-5 text-amber-400 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>
                                    <div class="ml-3">
                                        <p class="text-sm text-amber-700 font-bold">Pharmacy is currently offline.</p>
                                        <p class="text-xs text-amber-600 mt-1">Prescriptions will be queued but won't be dispensed until a Pharmacist logs in.</p>
                                    </div>
                                </div>
                            </div>
                            @endif
                                    ...
                                    <div class="flex items-start gap-3 p-3 rounded-xl border shadow-sm transition-all"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="flex items-center gap-3 mt-1 text-xs text-slate-600">
                                                <span><span x-text="item.amount"></span></span>
                                                <span>&bull; <span x-text="item.instruction"></span></span>
                            ...
                            <div class="bg-slate-50 border border-slate-200 rounded-xl p-5 relative">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div class="relative">
                                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">
                                            <span x-text="isOtcMode ? 'Medicine Name (type manually)' : 'Search RHU Inventory'"></span> <span class="text-rose-400">*</span>
                                        </label>
                                        <input type="text" x-model="searchQuery" 
                                               ...
                                               :placeholder="isOtcMode ? 'e.g. Biogesic, Neozep...' : 'Type to search (e.g. Paracetamol)...'">
                                        
                                        <!-- Suggestions Dropdown -->
                                        <div x-show="!isOtcMode && showSuggestions && suggestions.length > 0" 
                                             x-transition class="absolute z-50 w-full bg-white mt-1 border border-slate-200 rounded-xl shadow-xl max-h-56 overflow-y-auto" x-cloak>
                                                ...
                                                <div @click="selectMedicine(med)" class="px-4 py-3 hover:bg-teal-50 cursor-pointer border-b border-slate-100 last:border-0 transition group">
                                                    <div class="flex justify-between items-start">
                                                        <div>
                                                            ...
                                                            <p class="text-[11px] text-slate-500 mt-0.5" x-text="(med.generic_name || 'No generic') + (med.form ? ' · ' + med.form : '')"></p>
                                                        </div>
                                                        <span class="text-[10px] font-bold px-2 py-1 rounded-full shrink-0 ml-3"
                                ...
                                <div class="flex flex-wrap gap-1.5 mb-4">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mr-1 self-center">Quick:</span>
                                ...
                                <button type="button" @click="addPrescription()" 
                                        ...
                                        :disabled="!searchQuery.trim()" :class="!searchQuery.trim() ? 'opacity-50 cursor-not-allowed' : ''">
                                    ...
                                    <div x-data="{ 
                                        ...
                                        <div class="flex items-center justify-between mb-1.5">
                                            ...
                                            <div class="flex flex-wrap gap-1.5">
                                                <button type="button" @click="appendReason('Check Laboratory results')" class="text-[9px] font-bold bg-amber-100 text-amber-800 border border-amber-200 rounded px-1.5 py-0.5 hover:bg-amber-200">+ Labs</button>
                                                <button type="button" @click="appendReason('Monitor Vital Signs')" class="text-[9px] font-bold bg-amber-100 text-amber-800 border border-amber-200 rounded px-1.5 py-0.5 hover:bg-amber-200">+ Vitals</button>
                                                <button type="button" @click="appendReason('Follow-up Checkup')" class="text-[9px] font-bold bg-amber-100 text-amber-800 border border-amber-200 rounded px-1.5 py-0.5 hover:bg-amber-200">+ Routine</button>
                                            </div>
                                        </div>
                                        <textarea name="followup_reason" id="followup_reason" :required="isFollowUp" x-model="reason" rows="3" placeholder="e.g. Check lab results, monitor BP" class="w-full text-sm rounded-md border-amber-300 bg-white focus:border-amber-500 focus:ring-amber-500 text-amber-900 shadow-inner"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pt-4 border-t border-slate-100 mt-auto flex justify-end gap-3">
                            <button type="submit" 
                                    class="px-6 py-3 bg-teal-600 text-white border border-transparent rounded-lg font-extrabold shadow-lg shadow-teal-600/30 hover:bg-teal-700 hover:shadow-teal-700/40 transition flex items-center gap-2">
    ...
    <template x-teleport="body">
        <div x-show="genericConfirmOpen"
             ...
             aria-labelledby="nurse-confirm-modal-title"
             role="dialog"
             aria-modal="true">
                ...
                <div x-show="genericConfirmOpen"
                    ...
                    <div class="p-6 sm:p-8">
                        <div class="sm:flex sm:items-start">
                            <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-2xl sm:mx-0 sm:h-10 sm:w-10 bg-teal-100 dark:bg-teal-900/30 text-teal-600 dark:text-teal-400">
                                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                                </svg>
                            </div>
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-xl font-bold text-slate-900 dark:text-white" id="nurse-confirm-modal-title" x-text="genericConfirmTitle"></h3>
                                <div class="mt-2">
                                    <p class="text-sm text-slate-500 dark:text-slate-400" x-text="genericConfirmMessage"></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="bg-slate-50 dark:bg-slate-950/50 px-6 py-4 flex flex-row-reverse gap-3">
                        <button type="button"
                                ...
                                class="w-full sm:w-auto inline-flex justify-center rounded-xl border border-transparent shadow-lg px-6 py-2 bg-teal-600 hover:bg-teal-700 text-sm font-bold text-white transition-all cursor-pointer">
                            Confirm Completion
                        </button>
                        <button type="button"
                                @click="genericConfirmOpen = false"
