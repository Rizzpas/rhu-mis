    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-100 dark:bg-gray-900 transition-colors duration-200">
        ...
        <header class="bg-emerald-600 dark:bg-emerald-800 shadow-md z-20 sticky top-0"
            x-data="{
                ...
                showResults: false,
                links: [
                    { name: 'Vitals Station', route: '{{ route('triage.dashboard') }}', desc: 'Pre-triage queue & vitals recording', keywords: ['triage', 'vitals', 'blood pressure', 'temperature', 'weight', 'station'] },
                    { name: 'Clinical Nurse Dashboard', route: '{{ route('nurse.dashboard') }}', desc: 'Patient consultation overview', keywords: ['dashboard', 'home', 'overview', 'consultations', 'patients'] },
                    { name: 'Nurse Profile & Status', route: '{{ route('profile.edit') }}', desc: 'Account settings & duty toggle', keywords: ['profile', 'settings', 'duty', 'status', 'online', 'offline'] },
                ],
                get filteredLinks() {
                ...
                triggerSearch() {
                    window.dispatchEvent(new CustomEvent('triage-global-search', { detail: this.searchQuery }));
                },
                clearSearch() {
                    this.searchQuery = '';
                    this.triggerSearch();
                ...
                <div class="flex items-center gap-4 flex-1">
                    ...
                    <div class="relative w-full max-w-xl sm:ml-6 group">
                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                            <svg class="h-4 w-4 text-emerald-800/70 dark:text-emerald-300/80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                        </div>
                        <input type="text" 
                               x-model="searchQuery" 
                               @input="triggerSearch()"
                               ...
                               placeholder="Search patient, vitals queue, or shortcuts..." 
                        ...
                        <div x-show="showResults && searchQuery.length > 0" 
                            ...
                            <template x-if="filteredLinks.length === 0">
                                <div class="px-4 py-4 text-center">
                                    <p class="text-xs text-slate-600 dark:text-slate-300 font-medium">Filtering triage queue for "<span class="font-bold text-slate-900 dark:text-white" x-text="searchQuery"></span>"</p>
                                    <p class="text-[10px] text-slate-400 mt-1">Check matching waiting cards below</p>
                                </div>
                            </template>
                        </div>
                    </div>
