...
<body class="print:h-auto print:overflow-visible bg-gray-100 dark:bg-gray-900 flex h-screen overflow-hidden text-slate-800 dark:text-slate-100" x-data="{ 
    ...
    <!-- Main Content -->
    <div class="print:overflow-visible flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-100 dark:bg-gray-900 transition-colors duration-200">
        <!-- Top bar -->
        <header class="print:hidden bg-gradient-to-r from-emerald-600 to-emerald-800 dark:from-emerald-800 dark:to-emerald-950 shadow-md z-20 sticky top-0 transition-colors duration-200"
            x-data="{
                searchQuery: '',
                showResults: false,
                links: [
            ...
            <div class="flex justify-between items-center px-4 sm:px-6 py-3.5 gap-4">
                <div class="flex items-center gap-4 flex-1">
                    <button @click="sidebarOpen = true" class="md:hidden text-white hover:bg-emerald-700 dark:hover:bg-emerald-900 p-1.5 rounded-lg transition-colors">
                    ...
                    <div class="relative w-full max-w-xl sm:ml-6 group">
                        ...
                        <input type="text" x-model="searchQuery" @focus="showResults = true" @keydown.escape="showResults = false" placeholder="Quick search..." 
                            class="w-full pl-10 pr-4 py-2 bg-white rounded-full border-none shadow-inner text-sm text-emerald-900 placeholder-emerald-800/60 dark:bg-emerald-950 dark:text-emerald-100 dark:placeholder-emerald-400/50 transition-shadow">
                        ...
                        <div x-show="showResults && searchQuery.length > 0" 
                            ...
                            style="display: none;" 
                            class="absolute z-50 mt-2 w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden top-full left-0 py-2">
                                    ...
                                    <template x-for="link in filteredLinks" :key="link.name">
                                        <li>
                                            <a :href="link.route" class="block px-4 py-3 hover:bg-emerald-50 dark:hover:bg-slate-700/50 text-sm transition-colors group/item">
                                                <div class="flex items-center gap-3">
                                                    <div class="p-1.5 rounded-lg bg-emerald-100 dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 group-hover/item:scale-110 transition-transform">
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar bg-gray-100 dark:bg-gray-900">
            @include('partials.toast')
            @if ($errors->any())
                <div class="mb-4 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 p-4 rounded-2xl shadow-xs">
            ...
            @endif
            @yield('content')
            <footer class="mt-12 border-t border-gray-200 dark:border-gray-800 pt-6 text-center text-xs text-gray-500 dark:text-gray-400 pb-6">
                <p>&copy; {{ date('Y') }} Rural Health Unit - Silang. Information Desk Portal.</p>
            </footer>
        </main>
    </div>
