...
<div class="relative custom-dropdown-container inline-block w-full {{ $containerClass }}"
    ...
    <button type="button"
            ...
            @if($disabled) disabled @endif
            class="relative w-full flex items-center justify-between text-left rounded-xl border bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 shadow-2xs transition-all duration-150 cursor-pointer focus:outline-none focus:border-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed {{ $sizeClasses }} {{ $class }}"
            :class="open 
